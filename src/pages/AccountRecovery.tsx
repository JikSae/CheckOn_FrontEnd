...
export default function AccountRecovery() {
  ...
  useEffect(() => {
    if (type === "reset-password") setActiveTab("reset-password");
    else setActiveTab("find-id");
  }, [type]);
  ...
  return (
    <div className="flex items-center justify-center w-screen h-screen bg-[#E7E6E6] text-white font-sans">
      <div className="bg-[#FEFEFE] text-[#1F2937] rounded-2xl w-full max-w-2xl p-12 shadow-2xl space-y-8">
        <h1 className="text-4xl font-extrabold text-center mb-8">
          {activeTab === "find-id" ? "아이디 찾기" : "비밀번호 변경"}
        </h1>

        {/* 탭 전환 버튼 */}
        <div className="flex mb-8 rounded-lg overflow-hidden border border-gray-300 text-lg">
          <button
        ...
        {activeTab === "find-id" && (
          <form
            onSubmit={(e) => {
              e.preventDefault();
              // TODO: 백엔드로 닉네임 + 생년월일 정보 전송
              console.log("아이디 찾기 요청 전송됨");
            }}
            className="space-y-11 text-lg"
          >
            <div>
